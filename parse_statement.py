...
AMT_RE = re.compile(r"^\(?-?[\d,]+\.\d{2}\)?(Cr|Dr|CR|DR)?$")
...
def extract_transactions(pdf_path, status_file):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for pnum, page in enumerate(pdf.pages, 1):
            ...
            if pnum <= 2:
                all_page_text_sample += " " + " ".join(w["text"] for w in words)
            ...
            for ln in lines:
                ...
                if header and any(abs(w["top"] - list(current_header.values())[0]) < 2
                                   for w in ln_sorted) and pnum >= 1 and header is current_header:
                    pass  # header row detection re-runs each page; harmless to include check below
            ...
            for w in content_words:
                if date_x0 - 5 <= w["x0"] < particulars_x0 - 2 and is_date_token(w["text"]):
            ...
            for i, a_top in enumerate(anchors):
                ...
                narration = " ".join(buckets["particulars"]).strip()
